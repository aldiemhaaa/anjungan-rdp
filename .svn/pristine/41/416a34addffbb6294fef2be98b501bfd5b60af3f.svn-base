...
def showDpjpBpjsRj(request):
    kodePoli = request.POST.get('kodePoli')
    response = requests.get('http://10.1.0.6/rest-anjungan/api/referensi/bpjs/dpjp/pelayanan/2/spesialis/%s' % kodePoli)
    data = response.json()
    return JsonResponse(data)
...
def listPoliBpjs(request):
    response = requests.get('http://10.1.0.6/rest-anjungan/api/referensi/bpjs/poliklinik/list')
    data = response.json()
    return JsonResponse(data)

def listDokterPoli(request):
    id_poli = request.POST.get('id_poli')
    response = requests.get('http://10.1.0.6/rest-anjungan/api/referensi/dokter/poli/%s' % id_poli)
    data = response.json()
    return JsonResponse(data)

def checkPasienKontrol(request):
    no_bpjs = request.POST.get('no_bpjs')
    response = requests.get('http://10.1.0.6/rest-anjungan/api/pasien/kontrol/%s' % no_bpjs)
    data = response.json()
    return JsonResponse(data)